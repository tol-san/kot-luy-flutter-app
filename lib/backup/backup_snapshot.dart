...
class BackupSnapshot {
  ...
  factory BackupSnapshot.parse(String source) {
    ...
    for (final row in data['categories']) {
      ...
      if (!categoryIds.add(id)) {
        throw const FormatException('Duplicate category');
      }
      categories.add({
        'id': id,
        ...
        'is_archived': number(row['is_archived'] ?? 0, 0, 1),
      });
    }
  ...
  Future<void> restore(Database db) => db.transaction((txn) async {
    ...
    await txn.insert('restore_safety', {
      'id': 1,
      'snapshot': previous,
  ...
  static Future<BackupSnapshot?> safetyCopy(Database db) async {
    ...
    return rows.isEmpty
        ? null
        : BackupSnapshot.parse(rows.first['snapshot'] as String);
  }
}
